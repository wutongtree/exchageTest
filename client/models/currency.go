...
func AllCurrencys(createChan chan int) ([]Currency, error) {
	urlstr := getHTTPURL("api/currency")

	_, err := performHTTPGet(urlstr)
	if err != nil {
		createChan <- 1
...
// MyCurrencys MyCurrencys
func CurrencyId(id string, createChan chan int) (*Currency, error) {
	urlstr := getHTTPURL("api/currency/" + id)
...
func CreateCurrency(currency *Currency, createChan chan int) (string, error) {
	// Create New Fund
	urlstr := getHTTPURL("api/currency/create")
